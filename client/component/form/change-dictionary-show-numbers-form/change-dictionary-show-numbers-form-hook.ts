...
const SCHEMA = object({
  section: string().oneOf(DICTIONARY_NUMBER_MODES).required(),
  equivalent: string().oneOf(DICTIONARY_NUMBER_MODES).required()
});
...
export function useChangeDictionaryShowNumbers(dictionary: Dictionary): ChangeDictionaryShowNumbersSpec {
  const form = useForm<FormValue>(SCHEMA, {section: dictionary.settings.showSectionNumber, equivalent: dictionary.settings.showEquivalentNumber}, {});
  const request = useRequest();
  const {dispatchSuccessToast} = useToast();
  const handleSubmit = useMemo(() => form.handleSubmit(async (value) => {
    const response = await request("changeDictionarySettings", {number: dictionary.number, settings: {showSectionNumber: value.section, showEquivalentNumber: value.equivalent}});
